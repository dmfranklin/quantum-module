...
### Using Widgets in Exercises

To create an exercise that uses the widget, wrap the `<interactive>` tag in an `<exercise>` tag as follows:
...
This uses a type of exercise called a "dual" exercise. "Dual" exercises allow authors to indicate what to show in case JavaScript (which is required for interactivity) is not supported on the student's computer. This is highly unlikely anymore, but exists for legacy reasons. So, we place the `<interactive>` element in the `<dynamic>` section of the exercise, and place a text alternative in the `<static>` section of the exercise.
